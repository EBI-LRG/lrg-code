...
<xsl:variable name="updatable_set_desc">Mappings to genome assemblies and annotations from external sources</xsl:variable>
...
<xsl:variable name="section_annotation_border">
  <xsl:choose>
    <xsl:when test="$lrg_status=0">
      <xsl:text>section_annotation1</xsl:text>
    </xsl:when>
    <xsl:when test="$lrg_status=1">
      <xsl:text>lrg_pending_bc</xsl:text>
    </xsl:when>
    <xsl:when test="$lrg_status=2">
      <xsl:text>lrg_stalled_bc</xsl:text>
    </xsl:when>
  </xsl:choose>
</xsl:variable>
...
<xsl:variable name="section_annotation_bg">
  <xsl:choose>
    ...
    <xsl:when test="$lrg_status=1">lrg_pending_bg</xsl:when>
    <xsl:when test="$lrg_status=2">lrg_stalled_bg</xsl:when>
  </xsl:choose>
</xsl:variable>
            ...
            <!-- LRG ID + Gene name -->
              ...
              <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 padding-left-5 padding-right-0">
                ...
                <div class="input-group">
	                <input type="text" class="form-control ui-autocomplete-input search_record_input" size="33" id="search_id" onkeydown="javascript: if (event.keyCode==13) get_lrg_query();" autocomplete="off">
                  ...
                  <span class="input-group-btn search_record_button">
                    <button class="btn btn-search btn-search-record icon-search smaller-icon close-icon-0" type="button" onclick="javascript:get_lrg_query();"/>
                  </span>
                </div>
              </div>
...
<xsl:choose>
  ...
  <xsl:when test="$lrg_status=1">
    ...
    <div class="status_banner">
      <div class="lrg_pending_bg status_title">
  ...
  <xsl:when test="$lrg_status=2">
    <!-- Add a banner indicating that the record is pending if the pending flag is set -->
    <div class="status_banner">
      <div class="lrg_stalled_bg status_title">
 ...
 <div class="data_container container-extra" style="margin-top:155px">
      ...
      <div class="section_box">
          ...
          <table class="summary">
            ...
            <tbody>
              ...
              <xsl:if test="fixed_annotation/comment">
                <tr><td class="line_separator" colspan="6"></td></tr>
                <tr>
                  <td class="bold_font red">Note:</td>
                  <td class="external_link" colspan="5"><xsl:value-of select="fixed_annotation/comment"/></td>
                </tr>
              </xsl:if>
            </tbody>
          </table>
        ...
        <xsl:variable name="subsection_border">
          <xsl:choose>
            ...
            <xsl:when test="$lrg_status=1">lrg_pending_bc</xsl:when>
            <xsl:when test="$lrg_status=2">lrg_stalled_bc</xsl:when>
          </xsl:choose>
        </xsl:variable>
        
        <xsl:variable name="subsection_color_and_border">
          <xsl:choose>
            <xsl:when test="$lrg_status=0">main_subsection1</xsl:when>
            <xsl:when test="$lrg_status=1">lrg_pending</xsl:when>
            <xsl:when test="$lrg_status=2">lrg_stalled</xsl:when>
          </xsl:choose>
        </xsl:variable>
        ...
        <div>
          ...
          <div style="margin-top:0px;border:none">
            <xsl:attribute name="class">main_subsection</xsl:attribute>
            <span class="main_subsection icon-info smaller-icon"></span>
            <span class="main_subsection">Fixed reference sequences in this record</span>
          </div>
          ...
          <div class="section_content">
            <table class="summary">
              ...
              <xsl:if test="$count_tr!=0">
                ...
                <tr>
                  <td class="left_col">Number of sequences</td>
                  ...
                  <td class="right_col" colspan="2">
                    <b>Genomic: </b>1
                    <span class="lrg_blue bold_font" style="font-size:1.2em;padding:0px 10px">/</span>
                    <b>Transcript<xsl:if test="$count_tr &gt; 1">s</xsl:if>: </b><xsl:value-of select="$count_tr" />
                    <span class="lrg_blue bold_font" style="font-size:1.2em;padding:0px 10px">/</span>
                    <b>Protein<xsl:if test="$count_pr &gt; 1">s</xsl:if>: </b><xsl:value-of select="$count_pr" />
                  </td>
                </tr>
                <tr>
                  <td class="external_link" colspan="6">
  ...
  <xsl:if test="$lrg_status=1">
    <div class="status_banner">
      <div class="lrg_pending_bg status_title_bottom icon-alert">PENDING APPROVAL</div>
    </div>
  </xsl:if>
  <xsl:if test="$lrg_status=2">
    <div class="status_banner">
      <div class="lrg_stalled_bg status_title_bottom icon-alert">STALLED</div>
    </div>
  </xsl:if>
...
<xsl:template xmlns:xslt="https://www.w3.org/1999/XSL/Transform" name="for-loop-d1e144"> 
  <xsl:param name="i"/>
...
<!-- for-loop-d1e417 -->
<xsl:template xmlns:xslt="https://www.w3.org/1999/XSL/Transform" name="for-loop-d1e417">
  <xsl:param name="i"/>
...
<!-- for-loop-d1e966 -->
<xsl:template xmlns:xslt="https://www.w3.org/1999/XSL/Transform" name="for-loop-d1e966">
  <xsl:param name="i"/>
  ...
  <div id="fixed_annotation_div" class="section_div">
    ...
    <div>
      ...
      <xsl:if test="count(/*/fixed_annotation/transcript) &gt; 1">
        ...
        <!-- Information block -->
        <div class="seq_info_box_container clearfix">
        ...
        <tbody>
          <tr>
            ...
            <td class="right_col_fixed_width">
              <xsl:call-template name="thousandify"><xsl:with-param name="number" select="string-length(/*/fixed_annotation/sequence)"/></xsl:call-template> nt
            </td>
            <td>
  ...
  <div>
    <div class="lrg_transcript_button">
      <xsl:call-template name="show_hide_button">
        ...
        <xsl:with-param name="link_text">Full transcript sequence: <xsl:value-of select="$transname"/> (<xsl:value-of select="$tr_length"/>nt)</xsl:with-param>
        <xsl:with-param name="show_as_button">1</xsl:with-param>
      </xsl:call-template>
    </div>
      ...
      <div class="unhidden_content">
        ...
        <div style="display:none">
          ...
          <table border="0" cellpadding="0" cellspacing="0" class="sequence fasta">
            ...
            <xsl:call-template xmlns:xslt="https://www.w3.org/1999/XSL/Transform" name="for-loop-d1e417">
  ...
  <div>
    <div class="lrg_transcript_button">
      <xsl:call-template name="show_hide_button">
        ...
        <xsl:with-param name="link_text">Transcript coding sequence (CDS): (<xsl:value-of select="$cds_length"/>nt)</xsl:with-param>
        <xsl:with-param name="show_as_button">1</xsl:with-param>
      </xsl:call-template>
    </div>
  ...
  <div>
    <div class="lrg_transcript_button">
      <xsl:call-template name="show_hide_button">
        ...
        <xsl:with-param name="link_text">Translated sequence: <xsl:value-of select="$pepname"/> (<xsl:value-of select="$peptide_length"/>aa)</xsl:with-param>
        <xsl:with-param name="show_as_button">1</xsl:with-param>
      </xsl:call-template>
    </div>   
        ...
        <div class="left"> 
          ...
          <div style="display:none">
            ...
            <table border="0" cellpadding="0" cellspacing="0" class="sequence fasta">
              ...
              <xsl:call-template xmlns:xslt="https://www.w3.org/1999/XSL/Transform" name="for-loop-d1e966">
                <xsl:with-param name="i" select="1"/>
...
<xsl:template match="annotation_set">
  ...
  <div>
    <xsl:choose>
      <xsl:when test="@type!=$lrg_set_name">
        <xsl:attribute name="class">annotation_set annotation_set_ext</xsl:attribute>
      ...
      <xsl:otherwise><xsl:attribute name="class">annotation_set</xsl:attribute></xsl:otherwise>
    </xsl:choose>
        ...
        <span class="dotted_underline" data-placement="bottom" data-toggle="tooltip">
          <xsl:choose>
            <xsl:when test="$type='patch'">
              <xsl:attribute name="title">Fix patches represent changes to existing assembly sequences. These are generally error corrections (addressed by approaches such as base changes, component replacements/updates, switch point updates or tiling path changes) or assembly improvements, such as the extension of sequence into gaps.</xsl:attribute>
              Fix patch
            </xsl:when>
            <xsl:otherwise>
              <xsl:attribute name="title">Novel patches represent the addition of new alternate loci to the assembly. These are alternate sequence representations of sequence found on the chromosomes.</xsl:attribute>
              Novel patch
            </xsl:otherwise>
          </xsl:choose>
        </span>
      ...
      <table class="table table-hover table-lrg bordered">
        <thead>
           <tr class="top_th">
            ...
            <th class="split-header lrg_blue" colspan="2"><xsl:value-of select="$lrg_id"/> genomic coordinates</th>
  ...
  <table class="table bordered table-lrg">
    ...
    <tbody>
      ...
      <xsl:for-each select="/*/fixed_annotation/transcript[@name = $transname]/exon">
        ...
        <xsl:variable name="temp_lrg_current_ref_start">
          <xsl:choose>
            <xsl:when test="$current_ref_strand = 1">
              <xsl:value-of select="$current_ref_start + $lrg_start - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$current_ref_end - $lrg_start + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        
        <xsl:variable name="temp_lrg_current_ref_end">
          <xsl:choose>
            <xsl:when test="$current_ref_strand = 1">
              <xsl:value-of select="$current_ref_start + $lrg_end - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$current_ref_end - $lrg_end + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="lrg_current_ref_start">
          <xsl:call-template name="ref_coord">
            <xsl:with-param name="temp_ref_start" select="$temp_lrg_current_ref_start"/>
        ...
        <xsl:variable name="lrg_current_ref_end">
          <xsl:call-template name="ref_coord">
            <xsl:with-param name="temp_ref_start" select="$temp_lrg_current_ref_end"/>
            <xsl:with-param name="start_coord" select="$lrg_start"/>
            <xsl:with-param name="end_coord" select="$lrg_end"/>
            <xsl:with-param name="ref_assembly" select="$current_assembly"/>
          </xsl:call-template>
        ...
        <xsl:variable name="temp_lrg_previous_ref_start">
          <xsl:choose>
            <xsl:when test="$previous_ref_strand = 1">
              <xsl:value-of select="$previous_ref_start + $lrg_start - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$previous_ref_end - $lrg_start + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        
        <xsl:variable name="temp_lrg_previous_ref_end">
          <xsl:choose>
            <xsl:when test="$previous_ref_strand = 1">
              <xsl:value-of select="$previous_ref_start + $lrg_end - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$previous_ref_end - $lrg_end + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="lrg_previous_ref_start">
          <xsl:call-template name="ref_coord">
            <xsl:with-param name="temp_ref_start" select="$temp_lrg_previous_ref_start"/>
            <xsl:with-param name="start_coord" select="$lrg_start"/>
            <xsl:with-param name="end_coord" select="$lrg_end"/>
        ...
        <xsl:variable name="lrg_previous_ref_end">
          <xsl:call-template name="ref_coord">
            <xsl:with-param name="temp_ref_start" select="$temp_lrg_previous_ref_end"/>
            <xsl:with-param name="start_coord" select="$lrg_start"/>
            <xsl:with-param name="end_coord" select="$lrg_end"/>
            <xsl:with-param name="ref_assembly" select="$previous_assembly"/>
          </xsl:call-template>
    ...
    <tbody>
      ...
      <xsl:for-each select="/*/fixed_annotation/transcript[@name = $transname]/exon">
        ...
        <xsl:variable name="temp_lrg_current_ref_start">
          <xsl:choose>
            <xsl:when test="$current_ref_strand = 1">
              <xsl:value-of select="$current_ref_start + $coding_ref_start - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$current_ref_end - $coding_ref_start + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="temp_lrg_current_ref_end">
          <xsl:choose>
            <xsl:when test="$current_ref_strand = 1">
              <xsl:value-of select="$current_ref_start + $coding_ref_end - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$current_ref_end - $coding_ref_end + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="temp_lrg_previous_ref_start">
          <xsl:choose>
            <xsl:when test="$previous_ref_strand = 1">
              <xsl:value-of select="$previous_ref_start + $coding_ref_start - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$previous_ref_end - $coding_ref_start + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="temp_lrg_previous_ref_end">
          <xsl:choose>
            <xsl:when test="$previous_ref_strand = 1">
              <xsl:value-of select="$previous_ref_start + $coding_ref_end - 1"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$previous_ref_end - $coding_ref_end + 1"/>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:variable>
  ...
  <xsl:variable name="intron_pos"    select="format-number(((($tr_start - $seq_start) div $seq_length) * $image_width),0)" />
    ...
    <xsl:variable name="tooltip_font_colour">
      <xsl:choose>
        ...
        <xsl:otherwise>#88F</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="tooltip_colour"> &lt;span style="color:<xsl:value-of select="$tooltip_font_colour" />;font-weight:bold"&gt;</xsl:variable>
        ...
        <table class="table table-hover table-lrg bordered">
          <thead>
            <tr>
              ...
              <th class="split-header lrg_blue" style="width:14%" colspan="2">LRG genomic</th>
...
<xsl:template name="diff_table_content">
  ...
  <xsl:choose>
    <xsl:when test="$genomic_mapping">
      <xsl:attribute name="class">table table-hover table-lrg bordered gen_diff_table</xsl:attribute>
    </xsl:when>
    <xsl:otherwise>
      <xsl:attribute name="class">table table-hover table-lrg bordered lrg-diff</xsl:attribute>
    </xsl:otherwise>
  </xsl:choose>
      ...
      <xsl:when test="contains(../@coord_system,$current_assembly)"><xsl:value-of select="$current_assembly"/></xsl:when>
         ...
         <thead>
            <tr>
              ...
              <th class="lrg_col" title="LRG allele">LRG allele</th>
              <th class="lrg_col" title="LRG coordinates">LRG coordinates</th>
            <xsl:if test="$show_hgvs=1">
              <th title="HGVS notation on genome assembly sequence">
                <xsl:call-template name="assembly_colour_border">
                  <xsl:with-param name="assembly"><xsl:value-of select="$assembly_label"/></xsl:with-param>
                </xsl:call-template>
                Genomic HGVS <div class="smaller-font">(Forward strand &#8594;)</div>
              </th>
              <th title="HGVS notation on the transcript sequence">
                ...
                </xsl:call-template>Transcript HGVS 
              </th>
              <th class="lrg_col" title="HGVS notation on LRG sequence">LRG HGVS</th>
              <th title="List of the co-located variant on the genome">
  ...
  <xsl:if test="$chr='X' or $chr='Y' or $chr='MT' or number($chr)">
    ...
    <div class="hgvs nowrap">
        ...
        <a class="vep_icon" data-toggle="tooltip" data-placement="bottom" target="_blank">
          ...
          <xsl:attribute name="title">Click on the link above to see the VEP output for <xsl:value-of select="$chr"/><xsl:value-of select="$hgvs_gen"/></xsl:attribute>
        </a>
    </div>
  </xsl:if>
</xsl:template>
...
<xsl:template name="assembly_mapping">
  ...
  <h3>
    ...
    <span class="subsection">Mapping to the <xsl:if test="contains($assembly,$current_assembly)">Primary </xsl:if>Genome Assembly </span> <xsl:value-of select="$assembly"/>
...
<xsl:template name="genoverse">
 <!-- Genoverse button -->
    ...
    <div class="genoverse_button_line">
      <xsl:call-template name="show_hide_button">
        <xsl:with-param name="div_id">genoverse_div</xsl:with-param>
    ...
    <div>
      ...
      <div style="position:relative;left:5px">
        ...
        <xsl:variable name="main_chr" select="$current_mapping/@other_name"/>
        ...
        <script type="text/javascript">
          <xsl:attribute name="src"><xsl:value-of select="$lrg_extra_path"/>Genoverse/js/genoverse.combined.nojquery-ui.js</xsl:attribute>
          {
            ...
            start     : <xsl:value-of select="$current_ref_start"/>,
            end       : <xsl:value-of select="$current_ref_end"/>,
            plugins   : [ 'controlPanel', 'karyotype', 'trackControls', 'resizer', 'focusRegion', 'fullscreen', 'tooltips', 'fileDrop' ],
            tracksLibrary: [
              Genoverse.Track.extend({
                name       : '<xsl:value-of select="$current_assembly"/>',
              ...
              Genoverse.Track.File.DIFF.extend({
                name      : 'Sequence differences LRG vs <xsl:value-of select="$current_assembly"/>',
...
<div class="seq_info_header clearfix">
  ...
  <div class="left margin-left-5">Information</div>
</div>
</xsl:template>
