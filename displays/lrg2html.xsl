  ...
  <div class="menu">  
    ...
    <div class="download_box gradient_color1">
      ...
      <span class="icon icon-functional" data-icon="="></span>
        ...
        <a class="green_button" id="download_xml" data-info="File containing all the LRG data in a XML file">
                <xsl:attribute name="onmouseover">show_info('download_xml')</xsl:attribute>
                <xsl:attribute name="onmouseout">hide_info('download_xml')</xsl:attribute>
        ...
        <span style="margin-left:2px;margin-right:8px;color:#FFF;font-weight:bold">or</span>
	      ...
	      <a class="green_button" id="download_fasta" data-info="FASTA file containing the LRG genomic, transcript and protein sequences">
                <xsl:attribute name="onmouseover">show_info('download_fasta')</xsl:attribute>
                <xsl:attribute name="onmouseout">hide_info('download_fasta')</xsl:attribute>
...
<xsl:template name="source_header">
  ...
  <div class="source">
    ...
    <xsl:if test="$source_name!='LRG'">
      <a class="show_hide_anno">
        <xsl:attribute name="id">show_hide_anno_<xsl:value-of select="$source_id"/></xsl:attribute>
        <xsl:attribute name="href">javascript:showhide_anno('<xsl:value-of select="$source_id"/>');</xsl:attribute>Show annotations</a>
    </xsl:if>
  </div>
</xsl:template>
    ...
    <div style="float:left;margin-top:15px;margin-left:20px">
      <div class="key_right">
        ...
        <ul class="key">
          ...
          <li class="key">
            <span class="sequence"><span class="startcodon">START codon</span> / <span class="stopcodon">STOP codon</span> / <span class="genomic_utr">UTR region</span></span> of the 
            <a>
              <xsl:attribute name="href">#transcript_<xsl:value-of select="transcript[position() = 1]/@name"/></xsl:attribute>
              transcript <xsl:value-of select="transcript[position() = 1]/@name"/>
            </a>
      ...
      <div style="float:left">      
        <table>
          <tr>
           ...
           <xsl:variable name="cstart" select="coding_region/coordinates/@start"/>
           <xsl:variable name="cend" select="coding_region/coordinates/@end"/>
      ...
      <div style="float:left;margin-left:20px">
        <div class="key_right">
          ...
          <ul class="key">
            ...
            <li class="key">
              <span class="sequence"><span class="startcodon">START codon</span> / <span class="stopcodon">STOP codon</span> / <span class="utr">UTR region</span></span>
            </li>
      ...
      <td style="padding:0px">
        <table class="diff">
          <tr class="gradient_color3" >
            ...
            <th title="HGVS notation on genomic reference sequence and LRG sequence">HGVS</th>
...
<xsl:template name="diff_hgvs_genomic_ref">
  ...
  <xsl:for-each select=".">
    ...
    <div style="padding-bottom:1px;margin-bottom:1px;border-bottom:1px dotted #CCC">
      <div style="float:left">
      ...
      <div style="float:right">
        <a class="vep_icon">
          ...
          <xsl:attribute name="onmouseover">show_info('<xsl:value-of select="$key"/>')</xsl:attribute>
          <xsl:attribute name="onmouseout">hide_info('<xsl:value-of select="$key"/>')</xsl:attribute>
          <xsl:attribute name="data-info">Click on the link above to see the VEP output for <xsl:value-of select="$chr"/><xsl:value-of select="$hgvs_type"/><xsl:value-of select="$diff"/></xsl:attribute>
        </a>
      </div>
      <div style="clear:both"></div>
    </div>
    ...
    <xsl:if test="$assembly!='none' and $lrg_status=0">
      <div style="float:right">
        <a class="vep_icon">
          ...
          <xsl:attribute name="onmouseover">show_info('<xsl:value-of select="$key"/>')</xsl:attribute>
          <xsl:attribute name="onmouseout">hide_info('<xsl:value-of select="$key"/>')</xsl:attribute>
          <xsl:attribute name="data-info">Click on the link above to see the VEP output for <xsl:value-of select="$lrg_id"/><xsl:value-of select="$hgvs_type"/><xsl:value-of select="$diff"/></xsl:attribute>
        </a>
      </div>
    </xsl:if>
      <div style="clear:both"></div>
...
<xsl:template name="show_hide_button">
  ...
  <a class="show_hide">
    ...
    <xsl:attribute name="href">javascript:showhide('<xsl:value-of select="$div_id"/>');</xsl:attribute>
    <xsl:attribute name="title">Show/Hide data</xsl:attribute>
  </a>
</xsl:template> 

<xsl:template name="hide_button">hide</xsl:template> 
