...
# If the coding end is within this exon, save this exon id as end exon id and calculate end offset within the exon
        ...
        update_rows( [ qq{phase = '$phase'}, qq{end_phase = '$end_phase'} ],
          [qq{exon_id = '$exon_id'}], ['exon'] );
...
# Insert translation into db (if available, the gene could be non-protein coding)
    ...
    # Set the translation_id as the canonical_translation_id in transcript table
      update_rows(
        [qq{canonical_translation_id = '$translation_id'}],
        [qq{transcript_id = '$transcript_id'}],
        ['transcript']
      );
    }
  }
...
sub add_coord_system {
  ...
  if ( !defined($coord_system_id) ) {
    ...
    if ( !defined($rank) ) {
      ...
      $rank =
        ( $dbCore->dbc->db_handle->selectall_arrayref($stmt)->[0][0] + 1 );
    }
...
sub add_external_db {
  ...
  my $external_db_ids =
    get_rows( $dbname, 'db_name', 'external_db', 'external_db_id' );
  my $external_db_id;
  ...
  if ( !defined($external_db_ids) || scalar( @{$external_db_ids} ) == 0 ) {
    ...
    $external_db_id =
      $dbCore->dbc->db_handle->selectall_arrayref($stmt)->[0][0];
...
sub add_lrg_overlap {
  ...
  my $attrib_type_id;
  if ($partial) {
    $attrib_type_id = add_attrib_type(
      'GeneOverlapLRG',
      'Gene overlaps LRG',
'This gene is partially overlapped by a LRG region (start or end outside LRG)'
    );
  }
  else {
    $attrib_type_id =
      add_attrib_type( 'GeneInLRG', 'Gene in LRG',
      'This gene is contained within an LRG region' );
  }
...
#ÊLoop over the pairs array. For each mapping span, first get a chromosomal slice and project this one to contigs
  foreach my $pair ( sort { $a->[3] <=> $b->[3] } @{$pairs} ) {
    ...
    if ( $pair->[0] eq 'DNA' ) {
      die(
"Distance between query and target is not the same, there is an indel which shouldn't be there. Check the code!"
        )
...
# The LRG->contig mapping is straightforward if LRG and chromosome is the same orientation
  ...
  #ÊIf the LRG and chromosome are different orientation, the hits are backwards
        ...
        add_assembly_mapping( $q_seq_region_id, $ctg_seq_region_id, $lrg_start,
          $lrg_end, $ctg_start, $ctg_end, $ctg_strand );
      }
    }
...
# Else if this is a mismatch, put an rna_edit entry into the seq_region_attrib table
      ...
      add_object_attrib( $q_seq_region_id, $attrib_type_id, $value,
        'seq_region' );
    }
...
# If this is an insertion in the LRG, we add the sequence as a contig and put a mapping between the LRG and the new contig
      if ( $pair->[2] >= $pair->[1] ) {
        ...
        my $contig_sri =
          add_seq_region( $contig_name, $contig_csi, $contig_len );

        # Add DNA for this seq_region unless the database is not core.
        my $is_core =
          ( $dbCore->dbc->dbname() =~ m/_core_/ )
          ; #(fetch_rows(['meta_value'],['meta'],["meta_key='schema_type'"])->[0][0] eq 'core');
        add_dna( $contig_sri, $contig_seq ) if ($is_core);

        # Add a mapping between the LRG and the contig
        add_assembly_mapping( $q_seq_region_id, $contig_sri, $pair->[1],
          $pair->[2], 1, $contig_len, 1 );
      }
    }
  }
}
...
sub add_xref {
  ...
  my $external_db_ids =
    get_rows( $external_db_name, 'db_name', 'external_db', 'external_db_id' );
  if ( !defined($external_db_ids) || scalar( @{$external_db_ids} ) == 0 ) {
...
sub purge_db {
  ...
  if ( !defined($cs_id) ) {
    warn( "Could not find coordinate system $lrg_coord_system in "
        . $dbCore->dbc()->dbname() );
    $cs_id = -1;
  }
  my $ctg_coord_system_id = get_coord_system_id('contig');
  if ( !defined($ctg_coord_system_id) ) {
    warn( "Could not find coordinate system contig in "
        . $dbCore->dbc()->dbname() );
  ...
  my @seq_region_ids =
    @{ get_seq_region_ids( $lrg_name . '_', $ctg_coord_system_id ) };
  ...
  foreach my $seq_region_id (@seq_region_ids) {
    ...
    foreach my $object_type ( ( 'gene', 'exon', 'transcript' ) ) {
      ...
      my $object_ids =
        get_rows( $seq_region_id, 'seq_region_id', $object_type,
        $object_type . '_id' );
      while ( my $object_id = shift( @{$object_ids} ) ) {
        ...
        if ( $object_type eq 'transcript' ) {
          ...
          remove_row(
            [
              qq{ensembl_object_type = 'Translation'},
              qq{ensembl_id = $translation_id}
            ],
            'object_xref'
            )
            if defined($translation_id);
...
#ÊRemove xref objects linked to this object id. Does not remove the xref itself, just the row in object_xref
        remove_row(
          [
            qq{ensembl_object_type = '} . ucfirst($object_type) . qq{'},
            qq{ensembl_id = $oid}
          ],
          'object_xref'
        );
  ...
  my $xref_ids = fetch_rows(
    [qq{x.xref_id}],
    [ qq{xref x}, qq{external_db edb} ],
    [
      qq{x.external_db_id = edb.external_db_id},
qq{edb.db_name = 'LRG' OR edb.db_name = 'ENS_LRG_transcript' OR edb.db_name = 'ENS_LRG_gene'},
qq{x.display_label = '$lrg_name' OR x.display_label LIKE '$lrg_name\_t%' OR x.display_label LIKE '$lrg_name\_g%'}
    ]
  );
...
# If specified, remove EVERYTHING LRG-related unless there still are LRG entries left
  if ($purge_all) {
    my $seq_regions =
      get_rows( $cs_id, 'coord_system_id', 'seq_region', 'seq_region_id' );
    if ( scalar( @{$seq_regions} ) ) {
      warn(
"Seq regions belonging to LRG coordinate system still exist in database, will not clear LRG data!"
      );
      return;
    }
    ...
    my $ids = fetch_rows(
      ...
      ["meta"],
      [
        "meta_key = 'assembly.mapping' OR meta_key = 'lrg'",
        "meta_value LIKE '%lrg%'"
      ]
    );
    remove_row(
    ...
    $ids =
      fetch_rows( ["analysis_id"], ["analysis"], ["logic_name LIKE 'LRG_%'"] );
    remove_row(
      [
        "analysis_id = '"
          . join( "' OR analysis_id = '", map( $_->[0], @{$ids} ) ) . "'"
      ],
      "analysis_description"
    );
    remove_row(
