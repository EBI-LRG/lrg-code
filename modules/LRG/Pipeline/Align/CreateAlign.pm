...
sub run {
  ...
  if ($lrg_id && $lrg_id ne '') {
    ...
    `perl $run_dir/lrg-code/scripts/align/transcript_alignment_tool.pl -g $gene -o $align_dir/$gene.html -hd $data_files_dir -hf $havana_file -nh_dl -hgmd $hgmd_file -lrg $lrg_id`;
  }
  else {
    #`perl $run_dir/lrg-code/scripts/align/transcript_alignment_tool.pl -g $gene -o $align_dir/$gene.html`;
    `perl $run_dir/lrg-code/scripts/align/transcript_alignment_tool.pl -g $gene -o $align_dir/$gene.html -hd $data_files_dir -hf $havana_file -nh_dl -hgmd $hgmd_file`;
  }
}
1;
